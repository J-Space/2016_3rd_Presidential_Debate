...
df['new_date'] = [datetime.strptime(date.strftime('%Y %b %d, %H:%M'), '%Y %b %d, %H:%M') - timedelta(hours = 4)\
                  for date in df.date]
df['election'] = [1 if word_in_text(['#election'], text) else 0 for text in df.text]
df['election2016'] = [1 if word_in_text(['#election2016'], text) else 0 for text in df.text]
...
# matplotlib qt #plot in a separated window
fig2 = plt.figure()
df[df.election == 1].groupby('new_date').count()['text'][:-1].plot(label = '#election', color = 'blue')
df[df.election2016 == 1].groupby('new_date').count()['text'][:-1].plot(label = '#election2016', color = 'red')
plt.legend(title = '#hashtag')
plt.title('# of Tweets That Hashtagged #election & #election2016')
plt.xlabel('Time')
plt.ylabel('# of Tweets')


print("\n" + "The total # of tweets with '#debate':")
print(df[df.election == 1].shape[0])

print("\n" + "The total # of tweets with '#debate2016':")
print(df[df.election2016 == 1].shape[0])
